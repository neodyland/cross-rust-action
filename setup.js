...
await $\`${g} cc -target ${t.zigtarget} \${kept}\`;`,s=o.resolve(t.dir),r=o.join(s,"bin");if(!await d(r))await $(r,{recursive:!0});let i=o.join(r,"cross-rust-cc");await Bun.write(i,e),await c`chmod +x ${i}`.quiet();let u=`#!${t.sh}
export CC=${i}
${Bun.which("cargo")} +${t.toolchain} --config 'target.${t.rusttarget}.linker="${i}"' build --target ${t.rusttarget} $@`,n=o.join(r,"cross-cargo-build");return await Bun.write(n,u),await c`chmod +x ${n}`.quiet(),console.log(n),!0}var z=await w();process.exit(z?0:1);
