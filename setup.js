...
export CC=${e}
${Bun.which("cargo")} +${t.toolchain} --config 'target.${t.rusttarget}.linker="${e}"' --config 'target.${t.rusttarget}.rustflags=["-C", "linker=${e}"]' build --target ${t.rusttarget} $@`,n=s.join(r,"cross-cargo-build");return await Bun.write(n,u),await c`chmod +x ${n}`.quiet(),console.log(n),!0}var z=await w();process.exit(z?0:1);
